...
export const confirmRenewal = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: client } = await db
      .from("clients").select("id, plan_id, expiry_date, monthly_fee").eq("id", req.client_id).single();
    const planId = req.plan_id ?? client?.plan_id ?? null;
    let validity = 30;
    if (planId) {
      const { data: plan } = await db.from("plans").select("validity_days").eq("id", planId).maybeSingle();
    ...
    return { ok: true, expiry: newExpiry, days: validity, sync };
  });
